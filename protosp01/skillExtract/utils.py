...
def read_json(path, lastN=None):
    ...
    with open(path, "r", encoding='utf-8') as f:
        if lastN is None:
            ...
            lines = f.readlines()[-lastN:] # TODO fix, doesn't wrk because it's single line json)
    for line in lines:
...
def text_completion(prompt, model="text-davinci-003", return_text=True, model_args=None):
    if model_args is None:
        model_args = {}
...
class OPENAI():
    ...
    def run_gpt_df_extraction(self):
        ...
        for i, sample in enumerate(tqdm(self.data)):
            instruction_field = 'instruction_job' if 'vacancies' in self.args.datapath else 'instruction'
            input = PROMPT_TEMPLATES['extraction'][instruction_field] + "\n" + PROMPT_TEMPLATES['extraction']['6shots']
            # TODO nb of shots as argument
            input += "\nSentence: " + sample['sentence'] + "\nAnswer:"
            max_tokens = self.args.max_tokens

            prediction = self.run_gpt_sample(input, max_tokens=max_tokens).lower().strip()
    ...
    def run_gpt_df_matching(self):
        ...
        for i, sample in enumerate(tqdm(self.data)):
            ...
            for extracted_skill in sample['extracted_skills']:
                input = PROMPT_TEMPLATES['matching'][instruction_field] + "\n" + PROMPT_TEMPLATES['matching']['1shot']
                # TODO having definition or not in the list of candidates
                options_dict = {letter.upper(): candidate["name+example"] for letter, candidate in zip(list('abcdefghijklmnopqrstuvwxyz')[:len(sample["skill_candidates"][extracted_skill])], sample["skill_candidates"][extracted_skill])}
                options_string = " \n".join(letter + ": " + description for letter, description in options_dict.items())
                ...
                chosen_option = options_dict[chosen_letter] if chosen_letter in options_dict else "None"
    ...
    def run_gpt_sample(self, prompt, max_tokens):
        ...
        if self.args.model in CHAT_COMPLETION_MODELS:
            response = chat_completion([{"role": "user", "content": prompt}], model=self.args.model, return_text=True, model_args={
            ...
            response = text_completion(prompt, model=self.args.model, return_text=True, model_args={
                "temperature": self.args.temperature,
                "max_tokens": max_tokens,
                "top_p": self.args.top_p,
                "frequency_penalty": self.args.frequency_penalty,
...
def filter_subwords(extracted_skill, skill_names, splitter):
    ...
    for subword in subwords:
        matched_elements.extend(filter(lambda item: subword in item[1], enumerate(skill_names)))
...
def select_candidates_from_taxonomy(sample, taxonomy, skill_names, skill_definitions, splitter, max_candidates):
    ...
    if len(sample['extracted_skills'])>0:
        ...
        for extracted_skill in sample['extracted_skills']:
            ...
            rule1 = list(filter(lambda item: extracted_skill in item[1], enumerate(skill_names)))
            # 2) look for all taxonomy skill definitions with exact match of extracted skill name inside
            rule2 = list(filter(lambda item: extracted_skill in item[1], enumerate(skill_definitions)))
            ...
            if len(matching_rows)>10:
                ...
                matching_rows = matching_rows.sample(max_candidates)  # TODO what to do with more than N extracted skills ?
            elif len(matching_rows)==0: # TODO update now that ew don't have type level 3 anymore
                matching_elements = difflib.get_close_matches(extracted_skill, taxonomy["name+example"], cutoff=0.4, n=max_candidates)
                matching_rows = taxonomy[taxonomy["name+example"].isin(matching_elements)]
            if len(matching_rows)==0:
                print("No candidates found for skill", extracted_skill)
            sample['skill_candidates'][extracted_skill] = matching_rows[['unique_id', 'name+example']].to_dict('records')
    return sample

def exact_match(data, tech_certif_lang, tech_alternative_names, certification_alternative_names):
    ...
    word_sets = [set(tech_certif_lang[tech_certif_lang['Level 1']==categ]['Level 2']) for categ in categs]
    for sample in data:
        sentence = sample['sentence']
        for category, word_set in zip(categs, word_sets):
            ...
            matching_words = re.findall(r'\b(?:' + '|'.join(re.escape(word) for word in word_set).replace(r'\#', '#') + r')\b', sentence)
            sample[category] = matching_words
        
        tech_synonym_set = list(synonym_to_tech_mapping.keys())
        matching_synonyms = re.findall(r'\b(?:' + '|'.join(re.escape(word) for word in tech_synonym_set) + r')\b', sentence)
        ...
        matching_synonyms = re.findall(r'\b(?:' + '|'.join(re.escape(word) for word in certif_synonym_set) + r')\b', sentence)
        matching_certif = [synonym_to_certif_mapping[word] for word in matching_synonyms]
